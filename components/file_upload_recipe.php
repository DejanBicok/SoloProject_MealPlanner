...
function file_upload($photo, $source = "USER")
{
    ...
    if ($fileError == 4) {
        $result->ErrorMessage = "No recipe picture was chosen. You can add it later.";
        return $result;
    } else {
        if (in_array($fileExtension, $filesAllowed)) {
            if ($fileError === 0) {
                if ($fileSize < 500000000) { //500kb this number is in bytes
                    ...
                    $result->ErrorMessage = "This photo is bigger than the allowed 50000Kb. <br> Please choose a smaller one and update the product.";
                    return $result;
                }
            } else {                              
